...
public class Extractor {
	...
	public String create_structured_file(String SummarisedFile,String title) throws IOException {

		String StructuresSummary = "";

		String inputSent = "", line;

		FileReader summary_file = new FileReader(SummarisedFile);
		BufferedReader br = new BufferedReader(summary_file);
		...
		InputStream inputStream = new FileInputStream(
				"F:\\Project(Summarization)\\Summarization\\OpenNlp\\en-sent.bin");
		...
		for (int s = 0; s < sentences.length; s++) {
			...
			FileInputStream tokenModelIn = new FileInputStream(
					"F:\\Project(Summarization)\\Summarization\\OpenNlp\\en-token.bin");
			TokenizerModel tokenModel = new TokenizerModel(tokenModelIn);
			Tokenizer tokenizer = new TokenizerME(tokenModel);
			String tokens[] = tokenizer.tokenize(sentence);

			InputStream posModelIn = new FileInputStream(
					"F:\\Project(Summarization)\\Summarization\\OpenNlp\\en-pos-maxent.bin");
			...
			wordLoop: for (int i = 0; i < chunks.length;) {
				...
				while (title.toLowerCase().contains(tokens[i].toLowerCase()))
				{
		...
		FileReader structured_file = new FileReader("outA.txt");
		BufferedReader br1 = new BufferedReader(structured_file);
		while ((line = br1.readLine()) != null) {
			StructuresSummary += line;
			StructuresSummary += "\n";
		}
		br1.close();
		structured_file.close();
		return StructuresSummary;
	}
}
